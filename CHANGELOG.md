...
- В модуль EventsAPI добавлен метод позволяющих получать события при помощи api v3 с использованием PDQL-синтаксиса для текстового представления.
  Подобное представление позволяет задать в одной строке фильтр, набор требуемых полей событий в ответе, параметры
  сортировки и ограничение на количество возвращаемых событий.
  Пример:
  filter(event_src.host contains 'example.com' and in_subnet(src.ip, "4.8.15.0/16")) | select(time, event_src.host, src.ip) | sort(time desc) | limit(42)
