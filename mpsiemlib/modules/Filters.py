...
class Filters(ModuleInterface, LoggingHandler):
    ...
    def get_folders_list(self) -> dict:
        ...
        self.log.info(f'status=success, action=get_folders_list, msg="Got {len(self.__folders)!r} folders", '
    ...
    def create_event_filter_v2(self, filter_name: str, folder_id: str, params: dict) -> str:
        ...
        self.log.info(f'status=success, action=create_event_filter_v3, msg="Created filter {filter_id!r}", '
                      f'hostname={self.__core_hostname!r}')
        return filter_id
    ...
    def create_event_filter_v3(self, filter_name: str, folder_id: str, pdql_query: str) -> str:
        ...
        :param filter_name: имя создаваемого фильтра
        :param folder_id: ID директории, в которой создается фильтр
        :param pdql_query: поисковый запрос.
    ...
    def create_event_filter(self, filter_name: str, folder_id: str, pdql_query: str) -> str:
        if (
                float(
                    f"{self.__core_version.split('.')[0]}.{self.__core_version.split('.')[1]}"
                )
                < 26.1
        ):
    ...
    def get_filters_list(self) -> dict:
        ...
        self.log.info(f'status=success, action=get_filters_list, msg="Got {len(self.__filters)!r} filters", '
                      f'hostname={self.__core_hostname!r}')
    ...
    def get_filter_info_v2(self, filter_id: str) -> dict:
        ...
        self.log.info(f'status=success, action=get_filter_info_v2, msg="Got info for filter {filter_id!r}", '
                      f'hostname={self.__core_hostname!r}')
    ...
    def get_filter_info_v3(self, filter_id: str) -> dict:
        ...
        self.log.info(f'status=success, action=get_filter_info_v3, msg="Got info for filter {filter_id!r}", '
                      f'hostname={self.__core_hostname!r}')
