...
class Incidents(ModuleInterface, LoggingHandler):
    ...
    def get_incidents_list(
        ...
        self.log.debug(f'status=prepare, action=get_incidents_list, msg="Try to get incidents list", '
                       f'hostname={self.__core_hostname!r}, filters={filters!r}, begin={begin!r}, end={end!r}')

        if int(self.__core_version.split(".")[0]) <= 24:
            url = f"https://{self.__core_hostname}{self.__api_incidents_list}"
            time_from = datetime.fromtimestamp(begin, tz=pytz.timezone("UTC")).strftime(
                self.__time_format
            )
            time_to = datetime.fromtimestamp(end, tz=pytz.timezone("UTC")).strftime(
                self.__time_format
            )
        else:
            url = f"https://{self.__core_hostname}{self.__api_incidents_list_r25}"
            time_from = datetime.fromtimestamp(begin, tz=pytz.timezone("UTC")).strftime(
                self.__time_format_r25
            )
            time_to = datetime.fromtimestamp(end, tz=pytz.timezone("UTC")).strftime(
                self.__time_format_r25
            )
        ...
        self.log.info(f'status=success, action=get_incidents_list, msg="Query executed, response have been read", '
                      f'hostname={self.__core_hostname!r}, filter={filters!r}, lines={line_counter!r}')
        self.log.info(f'hostname={self.__core_hostname!r}, metric=get_incidents_list, '
    ...
    def __iterate_incidents(self, url: str, params: dict, offset: int, limit: int):
        ...
        if response is None or "incidents" not in response:
            self.log.error(f'status=failed, action=kb_objects_iterate, msg="Core data request return None or '
    ...
    def get_incident_info(self, incident_id: str) -> dict:
        ...
        """
        self.log.debug(f'status=prepare, action=get_groups, msg="Try to get incident info for {incident_id!r}", '
                       f'hostname={self.__core_hostname!r}')
        ...
        self.log.info(f'status=success, action=get_table_info, '
    ...
    def __load_events(self, incident_id):
        ...
        events_count = response.get("count") if response.get("count") is not None else 0
