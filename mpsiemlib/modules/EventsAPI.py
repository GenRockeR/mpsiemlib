...
class EventsAPI(ModuleInterface, LoggingHandler):
    ...
    def get_events_group_by_api_json(self, params: dict, group_ids=None) -> dict:
        ...
        """
        self.log.debug(f'status=prepare, action=get_events_group_by_api_json, '
                       f'msg="Try to aggregate events by query {params["filter"]["groupBy"]!r}", '
        ...
        if group_ids:
            ...
            url = f'https://{self.__core_hostname}{self.__api_events_aggregation}'
        ...
        self.log.info(f'status=success, action=get_events_group_by_api_json, '
                      f'msg="Query executed, response have been read", hostname={self.__core_hostname!r}, '
                      f'lines={line_counter!r}')
        self.log.info(f'hostname={self.__core_hostname!r}, metric=get_events_group_by_api_json, '
    ...
    def get_event_details(self, event_id, event_date) -> dict:
        ...
        url = f'https://{self.__core_hostname}{api_url}'
        response = exec_request(self.__core_session, url).json()

        if response is None or "event" not in response:
            self.log.error(f'status=failed, action=get_event_details, msg="Core data request return None or '
    ...
    def get_events_metadata(self):
        ...
        if response is None or "fields" not in response:
            self.log.error(f'status=failed, action=get_events_metadata, msg="Core data request return None or '
                           f'has wrong response structure", hostname={self.__core_hostname!r}')
    ...
    def get_events_grouped_by_fields(self, query_filter, group_by_fields, time_from, time_to) -> dict[str, int]:
        ...
        response = exec_request(self.__core_session, url, method="POST", json=params).json()

        if response is None or "rows" not in response:
            self.log.error(f'status=failed, action=get_events_grouped_by_fields, msg="Core data request return None or '
                           f'has wrong response structure", hostname={self.__core_hostname}')
            raise Exception("Core data request return None or has wrong response structure")

        return {
            " | ".join(str(s) for s in e["groups"]): int(e["values"][0])
    ...
    def get_events_by_filter(self, pdql_filter, fields, time_from, time_to, limit, offset) -> dict:
        ...
        if response is None or "events" not in response:
            self.log.error(f'status=failed, action=get_events_by_filter, msg="Core data request return None or '
                           f'has wrong response structure", hostname={self.__core_hostname!r}')
            raise Exception("Core data request return None or has wrong response structure")
    ...
    def get_events_for_incident(self, fields, incident_id, time_from, time_to, limit, offset):
        ...
        response = rq.json()
        if response is None or "events" not in response:
            self.log.error(f'status=failed, action=get_events_for_incident, msg="Core data request return None or '
    ...
    def get_events_by_filter_v3(self, query_filter: str, time_from: int, time_to: int) -> Iterator[dict]:
        ...
        """
        self.log.debug(f'status=prepare, action=get_events_by_filter_v3, msg="Try to get events", '
        ...
        while not is_end:
            ...
            for i in events:
                ...
                yield i
        took_time = get_metrics_took_time(start_time)
        self.log.info(f'hostname={self.__core_hostname!r}, metric=get_events_by_filter_v3, '
                      f'took={took_time:.4f} ms, objects={line_counter!r}')
